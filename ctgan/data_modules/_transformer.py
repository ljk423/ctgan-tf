...
class DataTransformer(object):
    ...
    cond_tensor: list[tf.Tensor]
        List of `tf.Tensor` describing the metadata needed for correctly compute
        the conditional loss in :func:`ctgan.losses.conditional_loss`.
    """
    ...
    def generate_tensors(self):
        ...
        - `output_tensor`: it is used in the :class:`ctgan.layers.GenActivation`
        ...
        - `cond_tensor`: it is used in the :func:`ctgan.losses.conditional_loss`
    ...
    def _fit_discrete(self, column, data):
        """Fits an One-Hot encoder to the corresponding data column, and returns
        a dictionary describing the obtained transformer.
    ...
    def fit(self, data, discrete_columns=tuple()):
        """Fits the data transformer to the input data, according to the passed
        `discrete_columns` list.

        For each continuous column :math:`C_i`, it will use variational Gaussian
        mixture model (VGM) to estimate the number of modes :math:`m_i` and fit
        a Gaussian mixture.

        For each discrete column :math:`d_i`, it will use an One Hot encoder to
        estimate the number of categories in the data.
    ...
    def inverse_transform(self, data, sigmas=None):
        """Inverse transforms data outputted by :class:`ctgan.models.Generator`,
        according to the metadata obtained from the original dataset in
        :meth:`ctgan.data_modules.DataTransformer.fit`.
        ...
        output = pd.DataFrame(output, columns=column_names).astype(self._dtypes)
